...
def main():
    ...
    st.set_page_config(page_title='ImmoEliza Real-Estate Price Predictor', layout='wide')

    # Load data and initialize app
    app = StreamlitApp(MODEL_PATH, COMMUNE_DATA_PATH, col_pair1=[1, 15], col_pair2=[1.2, 2])
    # Set layout
    app.set_layout()
    # Set input mask
...
class StreamlitApp:
    ...
    def __init__(self, model_path: str, location_data_path: str, col_pair1: List[Any], col_pair2: List[Any]):
        ...
        self.communes = sorted(
            Preprocessor.get_unique_values(column='commune', import_path=self.location_data_path)
        )

        # Initialize columns
        self.col1, self.col2, self.col3, self.col4 = self.create_columns(col_pair1, col_pair2)
    ...
    def predict_price(self, input_data: dict):
        ...
        try:
            ...
            preprocessed_data = self.preprocessor.preprocess(data=input_data, import_path=self.location_data_path)
            # Get prediction
    ...
    def display_map(self, selected_commune: str):
        ...
        commune_coordinates = self.commune_data[self.commune_data['commune'] == selected_commune][['latitude', 'longitude']]
